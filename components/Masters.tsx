...
const Masters: React.FC = () => {
  ...
  const handleAddTable = () => {
    ...
    if (tables.some(t => t.number === table.number)) {
        ...
        <div className="p-8">
          {activeTab === 'Items' && (
            <div className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-6 gap-4 bg-[#0f172a] p-6 rounded-3xl border border-slate-800">
                ...
                <div className="flex items-end">
                  <button onClick={handleAddItem} className="w-full h-[52px] bg-indigo-600 text-white font-black rounded-2xl hover:bg-indigo-500 shadow-xl shadow-indigo-900/30 transition-all uppercase text-[11px] tracking-widest flex items-center justify-center gap-2">
                    <i className="fa-solid fa-circle-plus text-sm"></i> Save Item
                  </button>
                </div>
              </div>
              ...
              <div className="overflow-x-auto rounded-3xl border border-slate-800 shadow-xl">
                <table className="w-full text-left">
                  ...
                  <tbody className="bg-[#1e293b]/30">
                    {menu.map(item => (
                      <tr key={item.id} className="border-b border-slate-800 hover:bg-slate-700/50 transition-colors">
          ...
          {activeTab === 'Waiters' && (
            <div className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-[#0f172a] p-6 rounded-3xl border border-slate-800">
                ...
                <div className="flex items-end">
                  <button onClick={handleAddWaiter} className="w-full h-[52px] bg-indigo-600 text-white font-black rounded-2xl hover:bg-indigo-500 shadow-xl transition-all uppercase text-[11px] tracking-widest flex items-center justify-center gap-2">
                    <i className="fa-solid fa-user-plus text-sm"></i> Save Waiter
                  </button>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {waiters.map(waiter => (
                  <div key={waiter.id} className="bg-[#0f172a] p-5 rounded-3xl border border-slate-800 flex justify-between items-center group hover:border-indigo-500 transition-all">
                    <div>
                      <h4 className="text-white font-black text-sm uppercase">{waiter.name}</h4>
                      <p className="text-slate-500 text-xs font-bold">{waiter.phone || 'No Phone'}</p>
                    </div>
                    <button onClick={() => removeWaiter(waiter.id)} className="w-10 h-10 rounded-2xl bg-rose-500/10 text-rose-500 border border-rose-500/20 flex items-center justify-center hover:bg-rose-500 hover:text-white transition-all">
          ...
          {activeTab === 'Groups' && (
            <div className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-[#0f172a] p-6 rounded-3xl border border-slate-800">
                ...
                <div className="flex items-end">
                  <button onClick={handleAddGroup} className="w-full h-[52px] bg-indigo-600 text-white font-black rounded-2xl hover:bg-indigo-500 shadow-xl transition-all uppercase text-[11px] tracking-widest flex items-center justify-center gap-2">
                    <i className="fa-solid fa-folder-plus text-sm"></i> Save Group
                  </button>
                </div>
              </div>
              <div className="flex flex-wrap gap-4">
                {groups.map(group => (
                  <div key={group.id} className="bg-[#1e293b]/50 px-6 py-4 rounded-3xl border border-slate-800 flex items-center gap-4 group hover:border-indigo-500 transition-all shadow-lg">
                    ...
                    <button onClick={() => removeGroup(group.id)} className="text-slate-500 hover:text-rose-500 transition-colors">
          ...
          {activeTab === 'Taxes' && (
            <div className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-[#0f172a] p-6 rounded-3xl border border-slate-800">
                ...
                <div className="flex items-end">
                  <button onClick={handleAddTax} className="w-full h-[52px] bg-indigo-600 text-white font-black rounded-2xl hover:bg-indigo-500 shadow-xl transition-all uppercase text-[11px] tracking-widest flex items-center justify-center gap-2">
                    <i className="fa-solid fa-percent text-sm"></i> Save Tax
                  </button>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {taxes.map(tax => (
                  <div key={tax.id} className="bg-indigo-900/10 border border-indigo-500/20 p-6 rounded-3xl flex flex-col items-center group hover:bg-indigo-900/20 transition-all relative">
                    ...
                    <button onClick={() => removeTax(tax.id)} className="w-8 h-8 rounded-full bg-rose-500/10 text-rose-500 hover:bg-rose-500 hover:text-white flex items-center justify-center transition-all border border-rose-500/20">
          ...
          {activeTab === 'Tables' && (
            <div className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-[#0f172a] p-6 rounded-3xl border border-slate-800">
                ...
                <div className="flex items-end">
                  <button onClick={handleAddTable} className="w-full h-[52px] bg-indigo-600 text-white font-black rounded-2xl hover:bg-indigo-500 shadow-xl transition-all uppercase text-[11px] tracking-widest flex items-center justify-center gap-2">
                    <i className="fa-solid fa-chair text-sm"></i> Add Table
                  </button>
                </div>
              </div>
                ...
                {tables.map(table => (
                  <div key={table.id} className="relative group aspect-square">
                    ...
                    <button 
                      ...
                      className="absolute -top-1 -right-1 w-7 h-7 bg-rose-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-lg z-10"
                    >
